...
def runfio(ip_list, i):
    #for x, i in zip (ip_list, range(1, 3)):
    connection = ssh(ip_list, "root" , "password")
    transport = paramiko.Transport(ip_list)
...
#    channel.exec_command("sh NVMe_Performance.sh -a VF" + str(i))
    connection.sendCommand("sh NVMe_Performance.sh -a VF" + str(i))
...
count=0
while count<len(ip_list):
    for i in xrange(1, 17):
        threading.Thread(target=runfio,args=(str(ip_list[count]),i)).start()
        time.sleep(0.7)
        count+=1
